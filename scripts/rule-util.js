...
module.exports = (function(rulesDir, cwd) {
    ...
    function _init() {
        const nowRules = _loadRules(__dirname + "/.." + RCR.snow_rules_dir, "./");
    ...
    const getRulesForThisScriptRecord = function(ruleNames, scriptRecord) {
        ...
        let rulesAfterEvaluatingCond = ruleNames.filter(rule => {
            let ruleConfigF = rulesMap.get(rule),
                nowRConditionFunction;
            if (typeof ruleConfigF == "function") {
                let nowRConfig = ruleConfigF();
                if (nowRConfig && nowRConfig["condition"]) nowRConditionFunction = nowRConfig["condition"];
                else return true;
            } else if (typeof ruleConfigF == "object") nowRConditionFunction = ruleConfigF.meta.conditionFunc || null;
            else return;
